...
imageUploadInput.addEventListener('change', async (event) => {
    ...
    if (file) {
        ...
        if (uploadedImageUrl) {
            inputImageLink.value = uploadedImageUrl; // Wstaw URL do pola tekstowego
            previewImage.src = uploadedImageUrl; // Ustaw podgląd obrazu
            previewModal.style.display = 'flex';
        } else {
            alert('Nie udało się przesłać obrazu. Spróbuj ponownie.');
        }
    } else {
        console.error('Nie wybrano pliku obrazu');
    }
});

// Obsługa linków ręcznych
